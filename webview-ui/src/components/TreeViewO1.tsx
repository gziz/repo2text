 ...
 *  - Force expansion if searching, so matched nodes are visible.
...
function buildDisplayTree(
  ...
  function cloneAndFilter(node: TreeNode): TreeNode | null {
    // If searching, keep node if it's in matchingIds or has a matched descendant
    ...
    const cloned: TreeNode = {
      ...
      // If searching => forcibly expanded, else respect user expansions
      expanded: matchingIds ? true : expandedNodeIds.has(node.id),
      selected: false,
      indeterminate: false,
    };
